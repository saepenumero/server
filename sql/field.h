...
protected:
  ...
  public:
    Converter_strtoll10(CHARSET_INFO *cs, const char *str, size_t length)
    {
      ...
      m_result= (*(cs->cs.ha->strtoll10))(&cs->cs, str, &m_end_of_num, &m_error);
      /*
...
public:
  ...
  int reset() override
  {
    charset()->cs.ha->fill(&charset()->cs,(char*) ptr, field_length,
                          (has_charset() ? ' ' : 0));
    return 0;
  }
  int store(const char *to,size_t length,CHARSET_INFO *charset) override;
  ...
  void calculate_interval_lengths(uint32 *max_length, uint32 *tot_length)
  {
    ...
    *max_length= *tot_length= 0;
    for (pos= interval->type_names, len= interval->type_lengths;
         *pos ; pos++, len++)
    {
      size_t length= charset->cs.ha->numchars(&charset->cs, *pos, *pos + *len);
      DBUG_ASSERT(length < UINT_MAX32);
