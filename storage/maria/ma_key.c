  ...
  for (keyseg= int_key->keyinfo->seg ; keyseg->type ;keyseg++)
  {
	...
	*key++= 0;				/* NULL in key */
    ...
    if (keyseg->flag & HA_SPACE_PACK)
    {
      ...
      {
        length= (uint) cs->cs.ha->lengthsp(&cs->cs, (const char*)pos, length);
      }
      else
      {
	...
	*key++ = *--pos;
    ...
    if (length > char_length)
      cs->cs.ha->fill(&cs->cs, (char*) key+char_length, length-char_length, ' ');
    key+= length;
  }
  ...
  for (keyseg=info->s->keyinfo[keynr].seg ; keyseg->type ;keyseg++)
  {
	...
	goto err;
      ...
      if (keyseg->type != (int) HA_KEYTYPE_NUM)
      {
        ...
        keyseg->charset->cs.ha->fill(&keyseg->charset->cs,
                                    (char*) pos + length,
