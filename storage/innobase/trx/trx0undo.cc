                                       ...
                                       mtr_t* mtr)
{
  ...
  alignas(4) byte free_start[4];
  mach_write_to_2(free_start, free + TRX_UNDO_LOG_XA_HDR_SIZE);
  memcpy_aligned<2>(free_start + 2, free_start, 2);
  static_assert(TRX_UNDO_PAGE_START + 2 == TRX_UNDO_PAGE_FREE,
                "compatibility");
  static_assert(!((TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_START) % 4), "alignment");
  /* A WRITE of 2 bytes is never longer than a MEMMOVE.
  So, let us WRITE 2+2 bytes instead of WRITE+MEMMOVE. */
  mtr->memcpy(*undo_page, my_assume_aligned<4>
	      (TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_START + undo_page->frame),
              free_start, 4);
  uint16_t prev_log= mach_read_from_2(TRX_UNDO_SEG_HDR + TRX_UNDO_LAST_LOG +
                                      undo_page->frame);
  alignas(4) byte state_last[4];
  mach_write_to_2(state_last, TRX_UNDO_ACTIVE);
  mach_write_to_2(state_last + 2, free);
