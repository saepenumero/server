...
int Rdb_key_def::unpack_utf8_str(
  ...
  while (src < src_end) {
    ...
    src += 2;
    int res = cset->cs.ha->wc_mb(&cset->cs, wc, dst, dst_end);
    DBUG_ASSERT(res > 0 && res <= 3);
    if (res < 0) return UNPACK_FAILURE;
    dst += res;
  }

  cset->cs.ha->fill(&cset->cs, reinterpret_cast<char *>(dst), dst_end - dst,
                    my_pad_char(cset));
  return UNPACK_SUCCESS;
}
...
void Rdb_key_def::pack_with_varchar_space_pad(
  ...
  const size_t trimmed_len = charset->cs.ha->lengthsp(
      &charset->cs, (const char *)field_var->ptr + field_var->length_bytes,
      value_length);
    ...
    uint *used_bytes)          // output number of bytes used
{
  ...
  for (uint ii = 0; ii < src_len; ii += 2) {
    ...
    int res = cset->cs.ha->wc_mb(&cset->cs, wc, dst + used, dst_end);
    DBUG_ASSERT(res > 0 && res <= 3);
    if (res < 0) {
      return UNPACK_FAILURE;
    }
  ...
  if (!rdb_mem_comparable_space[cs->number].get()) {
    ...
    if (!rdb_mem_comparable_space[cs->number].get()) {
      ...
      const size_t space_mb_len = cs->cs.ha->wc_mb(
          &cs->cs, (my_wc_t) my_pad_char(cs), space_mb, space_mb + sizeof(space_mb));
