                      ...
                      const uchar *ascii, const uchar *ascii_end)
{
  ...
  {
    int c_len;
    if ((c_len= json_cs->cs.ha->wc_mb(&json_cs->cs, (my_wc_t) *ascii,
                                      json, json_end)) > 0)
    {
      json+= c_len;
      ascii++;
                  ...
                  CHARSET_INFO *res_cs, uchar *res, uchar *res_end)
{
  ...
  {
    int c_len;
    if ((c_len= res_cs->cs.ha->wc_mb(&res_cs->cs, s.c_next, res, res_end)) > 0)
    {
      res+= c_len;
      continue;
    }
    if (c_len == MY_CS_ILUNI)
    {
      ...
      */
      if ((c_len= res_cs->cs.ha->wc_mb(&res_cs->cs, '?', res, res_end)) > 0)
      {
        res+= c_len;
        continue;
      }
                ...
                CHARSET_INFO *json_cs, uchar *json, uchar *json_end)
{
  ...
  while (str < str_end)
  {
    my_wc_t c_chr;
    int c_len;
    if ((c_len= str_cs->cs.ha->mb_wc(&str_cs->cs, &c_chr, str, str_end)) > 0)
    {
      ...
      {
        if ((c_len= json_cs->cs.ha->wc_mb(&json_cs->cs, c_chr, json, json_end)) > 0)
        {
          json+= c_len;
          continue;
        }
      ...
      if ((c_len= json_cs->cs.ha->wc_mb(&json_cs->cs, '\\', json, json_end)) <= 0 ||
          (c_len= json_cs->cs.ha->wc_mb(&json_cs->cs,
