...
print_table_data(MYSQL_RES *result)
{
  ...
  if (column_names)
  {
    ...
    for (uint off=0; (field = mysql_fetch_field(result)) ; off++)
    {
      ...
      size_t numcells= charset_info->cs.ha->numcells(&charset_info->cs,
                                                  field->name,
  ...
  while ((cur= mysql_fetch_row(result)))
  {
    ...
    for (uint off= 0; off < mysql_num_fields(result); off++)
    {
      ...
      */
      size_t visible_length= charset_info->cs.ha->numcells(&charset_info->cs, buffer, buffer + data_length);
